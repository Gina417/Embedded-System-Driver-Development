...
typedef enum {
    ...
    /* Interrupt modes */
    MODER_EXTI_RT  = 0x04, // External interrupt mode of rising edge trigger
    MODER_EXTI_FT  = 0x05, // External interrupt mode of falling edge trigger
    MODER_EXTI_RFT = 0x06  // External interrupt mode of rising and falling edge trigger
}MODER_t;
...
/* GPIO Interrupt */
static inline uint8_t Is_GPIO_EXTI_MODE(MODER_t mode){
    return ( (mode == MODER_EXTI_RT) || (mode == MODER_EXTI_FT) || 
             (mode == MODER_EXTI_RFT) );
}
/* GPIO Pin */
...
static inline uint8_t Is_valid_GPIO_MODE(MODER_t mode){
    return( (mode == MODER_INPUT) || (mode == MODER_OUTPUT) || 
            ...
            (mode == MODER_EXTI_RT) || (mode == MODER_EXTI_FT) ||
            (mode == MODER_EXTI_RFT) );
}
/* GPIO Otype */
